...
6. **Extract a shared `app/hooks/useCustomElementEvent.js`.** The final review (2026-09-19) found this "ref + `addEventListener`" pattern now hand-rolled in three shapes across three files: `useModalEvents` (`app.models.jsx:179`), `useAfterHide` (`app.products.jsx:231`), and the pagination wiring (`ProductIndex.jsx:301-312`) — and the `onDropRejected` defect fixed in this same wave (see below) was a missed instance of exactly this pattern, found only because someone happened to notice a dead prop rather than because tooling flagged it. A shared hook (element ref in, event name(s) + handler(s) in, subscribe/unsubscribe handled once) would make the next instance harder to get wrong, and would give the class of bug a single choke point to audit instead of three-plus independent call sites. **Not built as part of this fix wave** — this is a refactor of working code and deliberately out of scope for a bug-fix pass; track it as its own change.

**Un-migrated `onChange`-on-`s-*` risk: none remains** (re-verified 2026-09-19). `grep -rn "onChange=" app/routes app/components` still returns only `ModelPicker`'s own component prop (called from `app.products.jsx`); every listener actually bound to an `s-*` element in `ModelPicker.jsx` and `app.models.jsx` uses `onInput`. This closes the pre-existing prod bug Task 4's review flagged.

**This claim was true but too narrow, and the final review (2026-09-19) found a third instance of the broader class it didn't cover.** The real risk is not "un-migrated `onChange`" specifically — it is any JSX handler prop on an `s-*` custom element whose underlying DOM event is not in React 18's `simpleEventPluginEvents` registry, which a `grep onChange=` is structurally incapable of finding regardless of how it's phrased, because the dead prop can have any name (`onDropRejected`, `onPreviousPage`, `onNextPage` were the three found across this codebase so far — the first two already fixed before this rescore, `onDropRejected` fixed in this same fix wave after being missed by the original "none remains" sweep). Re-checked this time by parsing every `<s-*` JSX opening tag under `app/routes` and `app/components` (multiline-aware, not a single-line grep) and listing every distinct `onXxx=` prop actually present: only `onClick` and `onInput` remain, both of which are confirmed-working simple events. No further un-migrated custom-element handler of this class is currently known in the codebase, but the only reliable check for it is exactly this kind of full enumeration against `@shopify/polaris-types`, repeated whenever a new `s-*` prop is added — not a `grep` for one specific prop name.
